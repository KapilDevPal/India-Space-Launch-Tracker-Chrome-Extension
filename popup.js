    ...
    i18n: {
      getMessage: (key, placeholders) => {
        const defaultMessages = {
          "appName": "ISRO launches & news unofficial",
          "appDesc": "Track ISRO launches and space events",
          "appTitle": "ISRO launches & news unofficial",
  ...
  function updateLayoutForTab(tab) {
    ...
    if (tab === "settings") {
      ...
      if (tab === "upcoming" || tab === "completed") {
        ...
        // Default state (no tab selected): show hero card, hide search box, keep navigation options
...
function renderDashboard() {
  ...
  // Only show the main card on the upcoming tab. Hide it on completed/news/moon tabs.
...
function renderList(container, list, type) {
  ...
  list.forEach(launch => {
    ...
    const isTBD = (launch.status || "").toLowerCase() === "tbd";
    const isPast = new Date(launch.launch_date_end || launch.launch_date) <= Date.now() && !isTBD;
    const uiLang = chrome.i18n.getUILanguage();
    ...
    if (isPast) {
      ...
      timerStr = `<span class="launch-item-time-val" data-launchtime="${launch.launch_date_end || launch.launch_date}">${chrome.i18n.getMessage("news_loading") || "Loading..."}</span>`;
    }
...
function tickHeroCountdown() {
  ...
  // Update trajectory arc fill and dot
  const pathFill = document.getElementById("arcFill");
  const dot = document.getElementById("arcDot");
  if (pathFill && dot) {
    const totalDuration = 24 * 60 * 60 * 1000; // 24 hours window
    const progress = Math.max(0, Math.min(1, 1 - (diff / totalDuration)));
    try {
      const totalLength = pathFill.getTotalLength();
      pathFill.style.strokeDasharray = totalLength;
      pathFill.style.strokeDashoffset = totalLength * (1 - progress);
      const point = pathFill.getPointAtLength(totalLength * progress);
      dot.setAttribute("cx", point.x);
      dot.setAttribute("cy", point.y);
    } catch (e) {
      console.error("SVG trajectory animation error:", e);
    }
  }
...
function openMissionModal(launch) {
  ...
  const vehicle = launch.launcher || launch.description?.match(/(?:PSLV|GSLV|LVM3|SSLV|Vikram|Agnibaan)[-\w]*/i)?.[0] || "LVM3-M4 Heavy Lift";
  modalVehicle.textContent = vehicle;

  modalAgency.textContent = launch.company_name || 'ISRO';

  const orbit = launch.orbit || launch.description?.match(/(?:LEO|GTO|SSO|MEO|Lunar|Sun-Synchronous|SSO polar)[-\w]*/i)?.[0] || "Low Earth Orbit (LEO)";
  ...
  if (isTBD) {
    ...
    const isPast = new Date(launch.launch_date_end || launch.launch_date) <= Date.now();
  ...
  // Populate source web link button (fallback to Google Search if source_url is missing)
  if (modalSourceLink) {
    const targetUrl = launch.source_url || `https://www.google.com/search?q=ISRO+${encodeURIComponent(launch.mission_name)}`;
    modalSourceLink.href = targetUrl;
  }
